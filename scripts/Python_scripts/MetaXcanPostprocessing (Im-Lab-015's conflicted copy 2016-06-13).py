...
def sortTopGeneListWithSNPs(projectName):
    ...
    gwas_snp = pandas.read_csv('gwas_snp.txt', sep=r"\s+")
...
def bubbleplot(projectName):
    ...
    r("""
        ...
        for (i in 1:length(startSites)) 
        {
            ...
            subData <- subset(data, data$start > startSites[i] - 500000  & 
                data$start < startSites[i] + 500000 & data$chr==chrosome[i])
...
def regionplot(projectName):
    ...
    r("""
        ...
        # subset data 
        subData <- subset(data, data$start > startSites[i] - 500000  & data$start < startSites[i] 
            + 500000 & data$chr==chrosome[i])
...
def locuszoom_plot(projectName):
    ...
    gwas_lead_snp = r("""
        gwas_lead_snp <- read.csv('sorted_top_genes.csv') %>% 
          ...
          distinct() %>% 
          mutate(flank='1MB', run = 'yes', m2zargs= "showAnnot=F")
